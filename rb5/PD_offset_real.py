...
# 3. PD gains
# =========================================================
K_a_mat = np.diag([400.0, 400.0, 400.0])
zeta_a_mat = np.diag([17.0, 17.0, 17.0])
...
ORI_BOOST_FULL_DEG = 1.0

# 축별 최소 자세 모멘트 boost
# [roll-like, pitch-like, yaw-like] 방향으로 생각하면 됨
ORI_BOOST_M_MIN_VEC = np.array([0.45, 0.25, 0.55])
...
print(f"[INFO] ORI_BOOST_ENABLE = {ORI_BOOST_ENABLE}")
print(f"[INFO] ORI_BOOST_M_MIN_VEC = {ORI_BOOST_M_MIN_VEC}")
...
with mujoco.viewer.launch_passive(m, d) as viewer:
    while viewer.is_running():
        ...
        # ---------------------------------------------------------
        # Orientation PD + axis-wise minimum moment boost
        # ---------------------------------------------------------
        ori_err0 = (
        ...
        ori_err_norm = np.linalg.norm(ori_err0)

        if ori_err_norm > 1e-9:
            ori_dir = ori_err0 / ori_err_norm
        else:
            ori_dir = np.zeros(3)

        ori_deadband = np.deg2rad(ORI_BOOST_DEADBAND_DEG)
        ori_full = np.deg2rad(ORI_BOOST_FULL_DEG)

        s_ori = smoothstep(ori_deadband, ori_full, ori_err_norm)

        if ORI_BOOST_ENABLE:
            M_boost = s_ori * ORI_BOOST_M_MIN_VEC * ori_dir
        else:
            M_boost = np.zeros(3)
